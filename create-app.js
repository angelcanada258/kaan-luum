...
function createApp(dataRepository = repository) {
  ...
  app.post('/api/entrada', async (req, res, next) => {
    try {
      ...
      const adentro = await dataRepository.obtenerAdentroAhora(fecha);
      ...
      await dataRepository.insertarRegistro({
        folio,
        tipo,
        evento: 'entrada',
        timestamp,
  ...
  app.post('/api/salida', async (req, res, next) => {
    try {
      const folio = normalizarFolio(req.body?.folio);
      const fecha = fechaLocal();
      if (!folio) throw apiError(400, 'El folio es obligatorio.');

      const registros = await dataRepository.obtenerRegistrosPorFecha(fecha);
      ...
      const visitante = (await dataRepository
  ...
  app.get('/api/dashboard', async (req, res, next) => {
    try {
      const fecha = fechaLocal();
      const resumen = await dataRepository.obtenerResumenDia(fecha);
  ...
  app.get('/api/historial', async (req, res, next) => {
    try {
      ...
      const registros = calcularDuraciones(
        await dataRepository.obtenerRegistrosPorFecha(fecha)
      )
  ...
  app.get('/api/adentro', async (req, res, next) => {
    try {
      res.json(await dataRepository.obtenerAdentroAhora(fechaLocal()));
    } catch (error) {
      next(error);
    }
  });
  ...
  app.get('/api/exportar', async (req, res, next) => {
    try {
      const fecha = req.query.fecha || fechaLocal();
      if (!fechaValida(fecha)) throw apiError(400, 'La fecha no es válida.');

      const rows = [['Folio', 'Tipo', 'Evento', 'Fecha', 'Hora']];
      for (const registro of await dataRepository.obtenerRegistrosPorFecha(fecha)) {
        rows.push([
          registro.folio,
          registro.tipo,
          registro.evento,
